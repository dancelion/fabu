 ...
 * @param app  应用名，CMF的应用名
 */
function openAppUploadDialog(dialog_title, callback, extra_params, app) {
    Wind.css('artDialog');
    multi      = 0;
    filetype   = 'file';
    app        = app ? app : GV.APP;
    var params = '&multi=' + multi + '&filetype=' + filetype + '&app=' + app;
    Wind.use("artDialog", "iframeTools", function () {
        art.dialog.open(GV.ROOT + 'apps/Asset/webuploader?' + params, {
            title: dialog_title,
            id: new Date().getTime(),
            width: '600px',
            height: '350px',
 ...
 * @param app  应用名,CMF的应用名
 */
function uploadApp(dialog_title, input_selector, extra_params, app) {
    openAppUploadDialog(dialog_title, function (dialog, files) {
        $(input_selector).val(files[0].filepath);
        $(input_selector + '-preview').attr('href', files[0].preview_url);
